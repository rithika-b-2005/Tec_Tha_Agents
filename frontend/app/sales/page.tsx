...
import {
  ...
  ExternalLink, X, ChevronRight, Trash2, TrendingUp, ArrowLeft,
} from "lucide-react"
        ...
        <div className="max-w-7xl mx-auto flex items-start justify-between gap-4">
          <motion.div initial={fadeUp.hidden} animate={fadeUp.visible} transition={transition.base}>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h1 className="text-2xl font-semibold text-gray-900">Sales Agent</h1>
            </div>
            <p className="text-sm text-gray-400 mt-0.5 ml-9">
              AI-qualified sales prospects — pain points, pitches &amp; proposals
            </p>
          </motion.div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={refreshing}
              className="p-2 rounded-lg border border-[#e0e0e0] bg-white text-gray-500 hover:text-emerald-600 hover:border-emerald-300 transition-colors"
              title="Refresh"
            >
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-sm font-medium transition-colors ${
                filter === t.key
                  ...
                  : "bg-white border border-[#e0e0e0] text-gray-600 hover:border-emerald-400 hover:text-emerald-600"
              }`}
            >
              {t.label}
            </button>
          ...
          <div className="bg-white border border-[#e8edf5] rounded-2xl shadow-sm overflow-hidden">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center px-6">
                ...
                <p className="text-sm text-gray-400 mb-5">Find businesses ready to buy your product or service</p>
                <Button asChild className="rounded-xl text-white text-sm font-semibold h-9 px-4 gap-1.5" style={{ background: "#059669" }}>
                  <Link href="/sales/generate"><Plus className="w-4 h-4" /> Find Prospects</Link>
                </Button>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#f0f2f5]">
                      {["Company", "Contact", "Location", "Pain Point", "Score", "Status", ""].map(h => (
                    ...
                    {filtered.map(lead => (
                      <tr key={lead.id} className="hover:bg-[#fafafa] transition-colors">
                        ...
                        <td className="px-5 py-3.5 max-w-[160px]">
                          {lead.email
                            ? <p className="text-xs text-emerald-600 flex items-center gap-1 truncate"><Mail className="w-3 h-3 shrink-0" />{lead.email}</p>
                        ...
                        <td className="px-5 py-3.5 max-w-[200px]">
                          {lead.painPoint
                            ? <p className="text-xs text-gray-600 line-clamp-2">{lead.painPoint}</p>
                            : <span className="text-xs text-gray-300">—</span>
                          }
                        </td>
          ...
          <div className="relative z-10 bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">This will permanently remove all {leads.length} sales prospects. This cannot be undone.</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative z-10 w-full max-w-lg bg-white h-full shadow-xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide px-6 py-5 space-y-5">
              ...
              {(selected.painPoint || selected.salesPitch || selected.proposalSummary) && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Sales Brief</p>
                  <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 space-y-3">
                    {selected.painPoint && (
                      <div>
                        <p className="text-xs font-semibold text-emerald-700 mb-1">Pain Point</p>
                        <p className="text-sm text-gray-700">{selected.painPoint}</p>
                      </div>
                    )}
                    {selected.salesPitch && (
                      <>
                        ...
                        <div>
                          <p className="text-xs font-semibold text-emerald-700 mb-1">Sales Pitch</p>
                          <p className="text-sm text-gray-600">{selected.salesPitch}</p>
                        </div>
                      </>
                    )}
                    {selected.proposalSummary && (
                      <>
                        <Separator className="bg-emerald-100" />
                        <div>
                          <p className="text-xs font-semibold text-emerald-700 mb-1">Proposal Summary</p>
                          <p className="text-sm text-gray-600 italic">{selected.proposalSummary}</p>
                        </div>
                      </>
                    )}
                  </div>
