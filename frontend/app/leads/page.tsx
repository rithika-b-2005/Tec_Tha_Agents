...
import {
  ...
  Star, ExternalLink, X, ChevronRight, Trash2, ArrowLeft,
} from "lucide-react"
        ...
        <div className="max-w-7xl mx-auto flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={refreshing}
              className="p-2 rounded-lg border border-[#e0e0e0] bg-white text-gray-500 hover:text-[#276ef1] hover:border-[#276ef1] transition-colors"
              title="Refresh"
            >
            ...
            >
              <Link href="/leads/generate">
                <Plus className="w-4 h-4" /> Generate Leads
              </Link>
            </Button>
          </div>
        </div>
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-sm font-medium transition-colors ${
                filter === t.key
                  ...
                  : "bg-white border border-[#e0e0e0] text-gray-600 hover:border-[#276ef1] hover:text-[#276ef1]"
              }`}
            >
              {t.label}
            </button>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center px-6">
                <div className="w-12 h-12 rounded-full bg-[#eff6ff] flex items-center justify-center mb-3">
                  <Star className="w-6 h-6 text-[#276ef1]" />
                </div>
                <p className="text-gray-900 font-medium mb-1">
                  {filter === "all" ? "No leads yet" : `No ${filter} leads`}
                </p>
                <p className="text-sm text-gray-400 mb-5">
                  Generate your first batch of AI-powered leads
                </p>
                <Button
                  asChild
                  className="rounded-xl text-white text-sm font-semibold h-9 px-4 gap-1.5"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#f0f2f5]">
                      {["Name / Company", "Contact", "Location", "Source", "Score", "Status", ""].map(h => (
                    ...
                    {filtered.map(lead => (
                      <tr key={lead.id} className="hover:bg-[#fafafa] transition-colors">
                        ...
                        <td className="px-5 py-3.5 max-w-[180px]">
                          {lead.email
                            ? <p className="text-xs text-[#276ef1] flex items-center gap-1 truncate"><Mail className="w-3 h-3 shrink-0" />{lead.email}</p>
                        ...
                        <td className="px-5 py-3.5 max-w-[150px]">
                          ...
                          {lead.industry && <p className="text-xs text-gray-400 mt-0.5 truncate">{lead.industry}</p>}
                        </td>
                        <td className="px-5 py-3.5">
                          <p className="text-xs text-gray-500 whitespace-nowrap">{lead.source}</p>
                        </td>
                        <td className="px-5 py-3.5">
                          ...
                          <div className="flex items-center gap-2">
                            {lead.emailBody && (
                              <button
                                ...
                                className="text-xs text-[#276ef1] hover:underline flex items-center gap-1 whitespace-nowrap"
                              >
                            ...
                            {lead.website && (
                              <a
                                ...
                                className="text-gray-400 hover:text-[#276ef1] transition-colors"
                                title="Visit website"
                              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 flex flex-col gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900">Delete all leads?</p>
                <p className="text-sm text-gray-400 mt-0.5">This will permanently remove all {leads.length} leads. This cannot be undone.</p>
              </div>
            </div>
          ...
          <div className="relative z-10 w-full max-w-lg bg-white h-full shadow-xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide px-6 py-5 space-y-5">
              ...
              <div className="space-y-2 text-sm">
                {selected.email && (
                  <a href={`mailto:${selected.email}`} className="flex items-center gap-2 text-[#276ef1] hover:underline">
                    <Mail className="w-3.5 h-3.5" />{selected.email}
                  </a>
                )}
                {selected.phone && (
                  <p className="flex items-center gap-2 text-gray-600">
                    <span className="w-3.5 h-3.5 text-gray-400">📱</span>{selected.phone}
                  </p>
                )}
                {selected.location && (
                  <p className="flex items-center gap-2 text-gray-600">
                ...
                {selected.website && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-[#276ef1] hover:underline"
                  >
              ...
              {/* AI Research */}
              {(selected.companyBio || selected.automationOpportunity) && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">AI Research</p>
                  <div className="bg-[#eff6ff] border border-blue-100 rounded-xl p-4 space-y-2">
                    {selected.companyBio && (
                    ...
                    {selected.automationOpportunity && (
                      <>
                        ...
                        <p className="text-xs font-semibold text-[#276ef1]">Automation Opportunity</p>
                        <p className="text-sm text-gray-600 italic">{selected.automationOpportunity}</p>
                      </>
                    )}
                  </div>
                </div>
              ...
              {selected.emailBody && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Personalized Cold Email</p>
